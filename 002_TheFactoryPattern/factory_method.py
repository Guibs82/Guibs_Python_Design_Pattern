...
def connection_factory(filepath):
    """根据传入文件后缀名选择方法调用, 并返回生成对象"""
    if filepath.endswith('json'):
        connector = JSONConnector
    elif filepath.endswith('xml'):
        connector = XMLConnector
    else:
        raise ValueError('Cannot connect to {}'.format(filepath)) # {} 相当于占位符 [Cannot connect to data/person.sq3]
...
def connect_to(filepath):
    ...
    factory = None
    try:
        factory = connection_factory(filepath=filepath)
